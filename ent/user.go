...
type User struct {
	...
	// Myuuid holds the value of the "myuuid" field.
...
func (*User) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case user.FieldMyuuid, user.FieldName:
	...
	for i := range columns {
		...
		case user.FieldMyuuid:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field myuuid", values[i])
			} else if value.Valid {
				u.Myuuid = value.String
			}
		case user.FieldName:
			if value, ok := values[i].(*sql.NullString); !ok {
...
func (u *User) String() string {
	...
	builder.WriteString(", myuuid=")
	builder.WriteString(u.Myuuid)
